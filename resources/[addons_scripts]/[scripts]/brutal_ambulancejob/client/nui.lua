RegisterNUICallback("UseButton", function(data)
	...
	elseif data.action == 'MDTInvoices' then
		if data.type == 'get' then
            ...
            if closestPlayer ~= -1 and closestDistance < 3.0 then
				InvoicesTargetID = closestPlayer
			end

			local DataTable = vSERVER.GetInvoiceTypes(InvoicesTargetID)
	...
	elseif data.action == 'MedicerMenu' then
		if data.type == 'useitem' then
			local success = vSERVER.removeMedicerItems(data.item)
			if success then
				if data.you then
	...
	elseif data.action == 'JobMenu' then
		if data.id == 'carry' then
    		...
    		if closestPlayer ~= -1 and closestDistance < 3.0 then
				if not InCarry then
					InCarry = true
					TriggerServerEvent('brutal_ambulancejob:server:carry', closestPlayer)
		...
		elseif data.id == 'ecg' then
			if not DoesEntityExist(MedicerEcg) then
				...
				local Progression,Network = vRPS.CreateObject(Config.MedicItems['ecg'].prop,x, y, z+0.2)
		        if Progression then
			        MedicerEcg = LoadNetwork(Network)
		...
		elseif data.id == 'bag' then
			if not DoesEntityExist(MedicerBag) then
				...
				local Progression,Network = vRPS.CreateObject(Config.MedicItems['bag'].prop,x, y, z+0.2)
		        if Progression then
					MedicerBag = LoadNetwork(Network)
		...
		elseif data.id == 'spawn' then
			...
			if not IsEntityAttachedToAnyVehicle(closestObject) and not IsEntityAttachedToAnyObject(closestObject) then
				if not DoesEntityExist(closestObject) then
					...
					local cds = GetOffsetFromEntityInWorldCoords(PlayerPedId(), 0.0, 1.5, -0.98)
					local Progression,Network = vRPS.CreateObject('prop_ld_binbag_01',cds[1], cds[2], cds[3]+0.2)
		        	if Progression then
						str = LoadNetwork(Network)
				...
				if DoesEntityExist(closestObject) then
					if not IsEntityAttachedToAnyVehicle(closestObject) then
						if StretcherData.puton == nil then
							StretcherData.puton = closestPlayer
							TriggerServerEvent('brutal_ambulancejob:server:puton', closestPlayer)
						elseif closestPlayer == StretcherData.puton then
							StretcherData.puton = nil
							TriggerServerEvent('brutal_ambulancejob:server:puton', closestPlayer)
						else
							SendNotify(26)
