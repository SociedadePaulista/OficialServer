	...
	$.post("http://chest/Chest",JSON.stringify({}),(data) => {
		...
		for (let x = 1; x <= data["Slots"]; x++){
			...
			if (data.Chest[slot] !== undefined){
				...
				const item = `<div class="item populated" title="" style="background-image: url('nui://inventory/web-side/inventory/${v["index"]}.png'); background-position: center; background-repeat: no-repeat;" data-amount="${v["amount"]}" data-stack="${v["stack"]}" data-item-key="${v["key"]}" data-slot="${slot}" data-peso="${v["peso"]}" data-description="${v["desc"]}" data-slot="${slot}">
					<div class="nameItem">${v.name}</div>
					<div class="top">
						<div class="itemAmount">${v.display || ParseFormat(v.amount)+"x"}</div>
						<div class="itemWeight">${(v.peso*v.amount).toFixed(2)}</div>
