...
function routeStart()
	Citizen.CreateThread(function()
	    ...
	    while LocalPlayer["state"]["taxi"] do
			...
			if IsPedInAnyVehicle(Ped) and GetEntityModel(GetVehiclePedIsUsing(Ped)) == GetHashKey("vovotaxi") and GetEntityHealth(Ped) > 100 then
...
function Salary()
	...
	CreateThread(function()
		while LocalPlayer["state"]["taxi"] do
			...
			local Ped = PlayerPedId()
			if IsPedInAnyVehicle(Ped)  and GetEntityHealth(Ped) > 100 then
				if GetEntityModel(GetVehiclePedIsUsing(Ped)) == GetHashKey("vovotaxi") then
					if LocalPlayer["state"]["taxi"] then
