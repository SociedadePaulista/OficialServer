	...
	$.post("http://crafting/Request",JSON.stringify({ name: Name }),(data) => {
		...
		for (let x = 1; x <= 25; x++){
			...
			if (data.Inventory[slot] !== undefined){
				...
				const item = `<div class="item populated" title="" style="background-image: url('nui://inventory/web-side/inventory/${v["index"]}.png'); background-position: center; background-repeat: no-repeat;" data-amount="${v["amount"]}" data-stack="${v["stack"]}" data-item-key="${v["key"]}" data-slot="${slot}" data-peso="${v["peso"]}" data-description="${v["desc"]}">
		...
		for (let x = 1; x <= data.craftSlots; x++){
			...
			if (data.Crafting[slot] !== undefined){
				...
				for (let xx = 0; xx < requirements.length; xx++) {
					...
					requirementHtml +=
					"<img id='requirementImage' src='nui://inventory/web-side/inventory/" +
				...
				const item = `<div class='craftContainer'>
					<div class="craftItem populated" title="" style="background-image: url('nui://inventory/web-side/inventory/${v["index"]}.png'); background-position: center; background-repeat: no-repeat;" data-amount="${v["amount"]}" data-item-key="${v["key"]}" data-slot="${slot}" data-price="${v["price"]}" data-peso="${v["peso"]}" data-description="${v["desc"]}" data-slot="${slot}">
						<div class="nameItem">${v["name"]}</div>
						<div class="top">
